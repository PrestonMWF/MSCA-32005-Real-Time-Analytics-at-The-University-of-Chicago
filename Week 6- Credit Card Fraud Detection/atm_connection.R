...
## and to return ingeger signal to send back to server
Connect <- function(host, port, login, password, stream_name, event_handler, catch_handler_errors=TRUE) {
    ...
    repeat {
        raw_msg <- ReceiveRawMsg(con);
        event_msg <- ATM.Event$read(raw_msg);
        # check errors
        ...
        if ( event_msg$stream_end ) {
            ...
            # process new event from server
            signal <- NULL;
            tryCatch({
                signal <- event_handler(event_msg$visit_id, event_msg$atm_signal);
                # assert prob length and value
                stopifnot(is.numeric(signal), length(signal) == 1,
                          signal %in% 0:4 );
            }, error = function(e) {
                if (catch_handler_errors) {
                    problem <- paste("Error inside handler: ", e, 'Forcing signal=0 !', sep='');
                    message('!!!***   WARNING   ***!!!\n', 
                            problem,
                            '\n!!!*******************!!!');
                    current_problem_n <<- current_problem_n + 1;
            ...
            if (signal != 0)
            {
                message("Sending signal=", signal, " for visit_id=", event_msg$visit_id);
                signal_msg <- ATM.Signal$new(visit_id = event_msg$visit_id, 
                                             signal = signal);
                # send signal message
...
message("atm_connection.R sourced!");
